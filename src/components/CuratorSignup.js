...
const CuratorSignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Make API call to register the curator
      const response = await axios.post("http://localhost:2000/api/curators/register", formData);
      ...
      setTimeout(() => {
        window.location.href = "/CuratorLogin";
      }, 2000);
    } catch (error) {
      if (error.response) {
  ...
  return (
    <div className="curator-signup-container">
      <div className="curator-signup-card">
        <h1 className="curator-signup-title">Curator Sign Up</h1>
        {message && <div className="error-message">{message}</div>}
        <form className="curator-signup-form" onSubmit={handleSubmit}>
          {/* Name Field */}
          ...
          <button type="submit" className="curator-signup-btn">
            Sign Up
          </button>

          <p className="curator-login-redirect">
            Already have an account?{" "}
            <a href="/CuratorLogin" className="curator-login-link">
              Login
            </a>
          </p>
        </form>
